...
import 'package:medilens_ai/core/network/api_service.dart';
import 'package:medilens_ai/shared/models/chat_message.dart';
import 'package:medilens_ai/shared/models/medicine.dart';
import 'package:medilens_ai/shared/models/reminder.dart';

class AppStateProvider with ChangeNotifier {
  // User Profile (Fallback mock data)
  String _userName = "Alex Rivera";
  String _userEmail = "alex.rivera@medilens.com";
  final String _userAvatar = AppAssets.homeDashboard;
  ...
  void _loadFallbackData() {
    ...
    final paracetamol = Medicine(
      ...
      imageAsset: AppAssets.medicineDetails,
    ...
    _chatMessages.add(
      ChatMessage(
        text:
            "Hello Alex! I am your MediLens AI Pharmacist. Ask me any question about your medications, side effects, or schedule guidelines.",
        isUser: false,
  ...
  Future<void> loadDataFromBackend() async {
    try {
        ...
        for (var item in medsData) {
          _medicines.add(
            Medicine(
              ...
              activeAllergiesConflict:
                  item['active_allergies_conflict'] ?? false,
      ...
      notifyListeners();
      debugPrint(
        "AppStateProvider successfully synchronized with the backend REST API.",
      );
    } catch (e) {
      debugPrint(
        "Backend connection failed: $e. Falling back to local offline mock data.",
      );
    }
  }
  ...
  void addReminder(String name, String dosage, String time) async {
    ...
    final isConflict =
        name.toLowerCase().contains("penicillin") ||
        name.toLowerCase().contains("amoxicillin");
    ...
    final hasMedicine = _medicines.any(
    ...
    try {
      ...
      if (!hasMedicine) {
        await ApiService.addMedicine({
          ...
          'frequency': "Once daily",
          'instructions': "Take as prescribed.",
          'image_asset': AppAssets.medicineDetails,
    ...
    try {
      ...
      Timer(const Duration(milliseconds: 1500), () {
        ...
        if (query.contains("paracetamol")) {
          responseText =
              "Paracetamol (500 mg) is safe for general pain and fever. Avoid taking other cold/flu medications containing paracetamol at the same time.";
        } else if (query.contains("penicillin") || query.contains("allergy")) {
          responseText =
              "I see Penicillin listed in your profile allergies. You must avoid Penicillin, Amoxicillin, and other beta-lactam antibiotics.";
        } else {
          responseText =
              "I've analyzed your query. To ensure absolute safety, verify the label. Based on your profile, I don't see any immediate conflicts.";
        }
  ...
  Future<Medicine> simulateScanningProcess() async {
    ...
    final amoxicillin = Medicine(
      ...
      instructions:
          "Take with food to prevent stomach upset. Finish the full course.",
      imageAsset: AppAssets.medicineDetails,
      scannedDate: DateTime.now(),
      sideEffects: const [
    ...
    // 1. Add locally
    final alreadyAdded = _medicines.any(
      (m) => m.name.toLowerCase() == "amoxicillin",
    );
    if (!alreadyAdded) {
    ...
    try {
      if (!alreadyAdded) {
        await ApiService.addMedicine({
          ...
          'warnings': [
            "ALLERGY WARNING: You are allergic to Penicillin. Amoxicillin is a penicillin derivative.",
            "Take at evenly spaced intervals.",
          ],
